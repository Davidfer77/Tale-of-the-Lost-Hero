...
class Scenario2(State):
    def __init__(self):
        ...
        self.__max_score = cfg_item("scenario", "scenario2", "maximum_score")

        self.next_state = States.Scenario3

        self.__clock = pygame.time.Clock()

        with resources.path(cfg_item("scenario","scenario2", "path"), cfg_item("scenario", "scenario2", "filename")) as scenario2_image_path:
            self.__background1=pygame.image.load(scenario2_image_path).convert_alpha()
    ...
    def __spawn_enemy(self):
        if random.random() < cfg_item("scenario", "scenario2", "enemy_spawn_prob"):
            enemy_list = [EnemyType.Skeleton, EnemyType.Predator, EnemyType.Shadow, EnemyType.Devil]
            enemy_prob = [cfg_item("scenario", "scenario2", "enemy_spawn_weight", "skeleton"), cfg_item("scenario", "scenario2", "enemy_spawn_weight", "predator"), \
                          cfg_item("scenario", "scenario2", "enemy_spawn_weight", "shadow"), cfg_item("scenario", "scenario2", "enemy_spawn_weight", "devil")]
